...
import {TestResultFilesystemRepository} from '../test-result-filesystem-repository';
import {TestResult} from '@domains/test/test-types';
import {TestStatus} from '@domains/test/test-enums';
import fs from 'fs';
import path from 'path';

describe('TestResultFilesystemRepository', () => {
  const testResultsDir = path.join(__dirname, '__test_results__');
  let repository: TestResultFilesystemRepository;

  beforeEach(() => {
    if (!fs.existsSync(testResultsDir)) {
      fs.mkdirSync(testResultsDir, {recursive: true});
    }
    repository = new TestResultFilesystemRepository(testResultsDir, 100);
  });
