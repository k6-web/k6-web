...
import {TestResultMemoryRepository} from '../test-result-memory-repository';
import {TestResult} from '@domains/test/test-types';
import {TestStatus} from '@domains/test/test-enums';

describe('TestResultMemoryRepository', () => {
  let repository: TestResultMemoryRepository;

  beforeEach(() => {
    repository = new TestResultMemoryRepository();
  });
  ...
  describe('save', () => {
    ...
    it('should cleanup old results when max limit is reached', async () => {
      const smallRepo = new TestResultMemoryRepository(3);
